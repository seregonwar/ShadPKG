...
struct PKGEntry {
  ...
  u32_be filename_offset; // Offset into the filenames table (ID 0x200) where
                          // this file's name is located
...
public:
  ...
  bool Extract(const std::filesystem::path &filepath,
  ...
  u32 GetNumberOfFiles() { return fsTable.size(); }

  u64 GetPkgSize() { return pkgSize; }

  std::string GetPkgFlags() { return pkgFlags; }

  std::string_view GetTitleID() { return std::string_view(pkgTitleID, 9); }

  PKGHeader GetPkgHeader() { return pkgheader; }

  // Getter per le chiavi derivate
  const std::array<u8, 32> &GetDK3() const { return dk3_; }
  const std::array<u8, 32> &GetIVKey() const { return ivKey; }
  const std::array<u8, 256> &GetImgKey() const { return imgKey; }
  const std::array<u8, 32> &GetEkpfsKey() const { return ekpfsKey; }
  const std::array<u8, 16> &GetDataKey() const { return dataKey; }
  const std::array<u8, 16> &GetTweakKey() const { return tweakKey; }

  static bool isFlagSet(u32_be variable, PKGContentFlag flag) {
    return (variable) & static_cast<u32>(flag);
  }

  static constexpr std::array<std::pair<PKGContentFlag, std::string_view>, 10>
      flagNames = {{{PKGContentFlag::FIRST_PATCH, "FIRST_PATCH"},
