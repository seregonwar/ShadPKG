...
void VariableAnalysis::analyze() {
  ...
  for (const auto &bb : func_->basicBlocks) {
    for (const auto &instr : bb->instructions) {
      ...
      if (instr.opcode == IR::OpCode::MOV || instr.opcode == IR::OpCode::ADD) {
          if (!instr.operands.empty() && instr.operands[0].type == IR::Operand::Type::Register) {
              uint64_t reg = instr.operands[0].value;
              if (reg == X86_REG_RAX || reg == X86_REG_EAX || reg == X86_REG_AX || reg == X86_REG_AL) {
      ...
      for (size_t i = 0; i < instr.operands.size(); ++i) {
          ...
          if (op.type == IR::Operand::Type::Register) {
              ...
              bool isDest = (i == 0 && (instr.opcode == IR::OpCode::MOV || instr.opcode == IR::OpCode::LEA || instr.opcode == IR::OpCode::POP || instr.opcode == IR::OpCode::MOVSX || instr.opcode == IR::OpCode::MOVZX));
              
              if (isDest) {
                  assignedRegisters_.insert(reg);
              } else {
                  // It's a source usage. Is it a param register not yet assigned?
                  if (registerParams_.count(reg) && assignedRegisters_.find(reg) == assignedRegisters_.end()) {
  ...
  std::sort(detectedParams_.begin(), detectedParams_.end(), [](const auto& a, const auto& b) {
      return a.paramIndex < b.paramIndex;
  });
}

std::string VariableAnalysis::getParamForRegister(uint64_t regId) const {
    if (regToParam_.count(regId)) return regToParam_.at(regId);
    return "";
}
